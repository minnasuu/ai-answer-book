...
// 静态文件服务 - 服务前端构建文件，支持子路径
app.use('/ai-answer-book', express.static(path.join(__dirname, 'dist'), {
  maxAge: '1d',
  etag: true,
  setHeaders: (res, path) => {
...
const proxyOptions = {
  ...
  timeout: 30000,
  pathRewrite: {
    '^/ai-answer-book/api': '' // 移除/ai-answer-book/api前缀
  },
...
// 处理预检请求
app.options('/ai-answer-book/api/*', (req, res) => {
  res.header('Access-Control-Allow-Origin', req.headers.origin || '*');
  res.header('Access-Control-Allow-Methods', 'GET,PUT,POST,DELETE,OPTIONS');
  res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, Content-Length, X-Requested-With');
  res.header('Access-Control-Allow-Credentials', 'true');
...
// 设置API代理中间件
app.use('/ai-answer-book/api', createProxyMiddleware(proxyOptions));

// 健康检查端点
app.get('/ai-answer-book/health', (req, res) => {
  res.json({ 
    status: 'ok', 
    timestamp: new Date().toISOString(),
    service: 'AI答案之书',
...
// API状态检查
app.get('/ai-answer-book/api-status', (req, res) => {
  res.json({
    api_proxy: '/ai-answer-book/api/openapi/v1/chat/completions',
...
// SPA路由处理 - 处理子路径下的所有请求
app.get('/ai-answer-book/*', (req, res) => {
  res.sendFile(path.join(__dirname, 'dist', 'index.html'));
});

// 根路径重定向到子路径
app.get('/', (req, res) => {
  res.redirect('/ai-answer-book/');
});
